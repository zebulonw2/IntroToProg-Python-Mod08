...
class FileProcessor:
    ...
    @staticmethod  # TODO add exception if .txt has non-float values for price, if either name or price is blank
...
Product names cannot be numeric, please check the file and run the program again
                ...
                try:
                    price = float(str(price).lstrip("$").rstrip()).__round__(2)  # change to string to remove both $ and \n
...
# class InputPriceNotFloatError(Exception):
#     """When adding new products, the price entered must contain only numbers and a max of one decimal"""
#     def __str__(self):
#         return "Prices must be numbers, please try again"
#
# class InputNameCannotBeNumbersError(Exception): # TODO this goes in setter
#     """Names entered by the user must not be numeric"""
#     def __str__(self):
#         return "Names cannot be numbers, please try again"
